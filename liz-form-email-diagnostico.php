...
function lizform_page_settings() {
    ...
    <style>
        ...
        .liz-row input[type=text],.liz-row input[type=email],.liz-row input[type=url],.liz-row input[type=number],.liz-row textarea{
        ...
        <form method="post" action="<?= esc_url( admin_url( 'admin-post.php' ) ) ?>">
            ...
            <div class="liz-sec">
                ...
                <div class="liz-row">
                    ...
                    <div>
                        <input type="email" name="notify_email" value="<?= esc_attr($s['notify_email'] ?? 'ivanenriqueales@gmail.com') ?>" placeholder="email@exemplo.com">
                        <p class="liz-note">Este e-mail recebe uma cópia com todas as respostas quando uma inscrição é salva.</p>
                        <?php $last_email_status = get_option( 'liz_form_last_email_status', '' ); ?>
                        <?php if ( $last_email_status ) : ?>
                            <p class="liz-note" style="color:#555;margin-top:8px"><strong>Status do último envio:</strong> <?= esc_html( $last_email_status ) ?></p>
                        <?php endif; ?>
                        <p style="margin-top:10px">
                            <a class="button" href="<?= esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=lizform_test_email' ), 'lizform_test_email' ) ) ?>">Enviar e-mail de teste</a>
                        </p>
                        <p class="liz-note">Use este botão para testar o envio sem preencher o formulário.</p>
                    </div>
                </div>
            </div>
...
function lizform_save_settings() {
    ...
    $new['notify_email'] = sanitize_email( wp_unslash( $_POST['notify_email'] ?? ( lizform_defaults()['notify_email'] ?? '' ) ) );
...
// ═══════════════════════════════════════════════════════
// DIAGNÓSTICO DO ENVIO DE E-MAIL DO WORDPRESS
// ═══════════════════════════════════════════════════════
add_action( 'wp_mail_failed', function( $wp_error ) {
    $msg = is_wp_error( $wp_error ) ? $wp_error->get_error_message() : 'Erro desconhecido no wp_mail.';
    update_option( 'liz_form_last_email_status', current_time( 'd/m/Y H:i:s' ) . ' — ERRO wp_mail: ' . $msg );
    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
        error_log( '[Liz Form] ERRO wp_mail: ' . $msg );
    }
}, 10, 1 );

if ( has_action( 'wp_mail_succeeded' ) !== false || function_exists( 'add_action' ) ) {
    add_action( 'wp_mail_succeeded', function( $mail_data ) {
        ...
        update_option( 'liz_form_last_email_status', current_time( 'd/m/Y H:i:s' ) . ' — wp_mail processou o envio' . ( $to ? ' para ' . $to : '' ) . '. Se não chegou, o bloqueio é de entrega/SMTP.' );
    }, 10, 1 );
}

// ═══════════════════════════════════════════════════════
// E-MAIL — helper robusto para notificação
// ═══════════════════════════════════════════════════════
function lizform_get_safe_from_email() {
    $host = wp_parse_url( home_url(), PHP_URL_HOST );
    $host = $host ? preg_replace( '/^www\./', '', strtolower( $host ) ) : '';

    if ( ! $host || ! is_email( 'no-reply@' . $host ) ) {
        $admin_email = get_option( 'admin_email' );
        return is_email( $admin_email ) ? $admin_email : 'wordpress@localhost.localdomain';
    }

    return 'no-reply@' . $host;
}

function lizform_update_email_status( $message ) {
    update_option( 'liz_form_last_email_status', current_time( 'd/m/Y H:i:s' ) . ' — ' . $message );
}

function lizform_send_notification_email( $submission_id, array $data ) {
    $s  = lizform_get();
    $to = sanitize_email( $s['notify_email'] ?? 'ivanenriqueales@gmail.com' );

    if ( ! $to || ! is_email( $to ) ) {
        lizform_update_email_status( 'E-mail de notificação inválido ou vazio.' );
        return false;
    }

    $nome_raw = sanitize_text_field( $data['nome'] ?? '' );
    $subject  = 'Nova inscricao - (R)Evolucao da Palavra - ' . ( $nome_raw ?: 'sem nome' );
    ...
    if ( ! empty( $s['f_nome_on'] ) )   $body .= "Nome:       " . sanitize_text_field( $data['nome']      ?? '-' ) . "\n";
    if ( ! empty( $s['f_insta_on'] ) )  $body .= "Instagram:  " . sanitize_text_field( $data['instagram'] ?? '-' ) . "\n";
    if ( ! empty( $s['f_email_on'] ) )  $body .= "E-mail:     " . sanitize_email(      $data['email']     ?? '-' ) . "\n";
    if ( ! empty( $s['f_cidade_on'] ) ) $body .= "Cidade/UF:  " . sanitize_text_field( $data['cidade']    ?? '-' ) . "\n";
    if ( ! empty( $s['f_idade_on'] ) )  $body .= "Idade:      " . sanitize_text_field( $data['idade']     ?? '-' ) . "\n";
    if ( ! empty( $s['f_prof_on'] ) )   $body .= "Profissao:  " . sanitize_text_field( $data['profissao'] ?? '-' ) . "\n";
    ...
    if ( $q_labels ) {
        ...
        foreach ( $q_labels as $qi => $ql ) {
            $resp  = sanitize_textarea_field( $data[ 'pergunta_' . $qi ] ?? '-' );
            $body .= "\n[Pergunta {$qi}] {$ql}\n-> {$resp}\n";
        }
    }
    ...
    $body .= "ID interno: #" . intval( $submission_id ) . "\n";
    $body .= "Painel:     " . admin_url( 'admin.php?page=liz-form' ) . "\n";

    $from_email = lizform_get_safe_from_email();
    $reply_to   = sanitize_email( $data['email'] ?? '' );

    $headers = [
        'Content-Type: text/plain; charset=UTF-8',
        'From: Liz Form <' . $from_email . '>',
    ];

    if ( $reply_to && is_email( $reply_to ) ) {
        $headers[] = 'Reply-To: ' . sanitize_text_field( $data['nome'] ?? 'Inscrição' ) . ' <' . $reply_to . '>';
    }

    // Define Return-Path/Envelope Sender com o mesmo domínio do site.
    $set_sender = function( $phpmailer ) use ( $from_email ) {
    ...
    if ( $sent ) {
        lizform_update_email_status( 'wp_mail aceitou o envio para ' . $to . '. Se não chegou na caixa, verifique spam ou configure SMTP.' );
        return true;
    }

    // Fallback final com mail() nativo do PHP. Ajuda em alguns servidores, mas SMTP ainda é o recomendado.
    $raw_headers = implode( "\r\n", $headers );
    $encoded_subject = '=?UTF-8?B?' . base64_encode( $subject ) . '?=';
    $fallback_sent = false;

    if ( function_exists( 'mail' ) ) {
        $fallback_sent = @mail( $to, $encoded_subject, $body, $raw_headers, '-f' . $from_email );
    }

    if ( $fallback_sent ) {
        lizform_update_email_status( 'wp_mail falhou, mas o fallback mail() aceitou o envio para ' . $to . '.' );
        return true;
    }

    lizform_update_email_status( 'Falha no envio: wp_mail e fallback mail() retornaram falso. Configure SMTP no WordPress.' );
    return false;
}

// ═══════════════════════════════════════════════════════
// TESTE MANUAL DE E-MAIL NO ADMIN
// ═══════════════════════════════════════════════════════
add_action( 'admin_post_lizform_test_email', 'lizform_test_email' );
function lizform_test_email() {
    check_admin_referer( 'lizform_test_email' );
    if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Sem permissão.' );
    ...
    $data = [
        ...
        'cidade'     => 'Teste',
        'idade'      => '00',
        'profissao'  => 'Teste de envio',
        'pergunta_1' => 'Este é um e-mail de teste gerado pelo botão do plugin.',
        'pergunta_2' => 'Se ele não chegar, é necessário configurar SMTP no WordPress.',
        'pergunta_3' => '',
        'pergunta_4' => '',
        'pergunta_5' => '',
    ];

    lizform_send_notification_email( 0, $data );
    wp_redirect( admin_url( 'admin.php?page=liz-form-settings&saved=1' ) );
    exit;
}
...
function lizform_api_submit( WP_REST_Request $req ) {
    ...
    $ok = $wpdb->insert( $wpdb->prefix . 'liz_submissions', $data );
    ...
    // ═══════════════════════════════════════════════════
    $email_sent = lizform_send_notification_email( $insert_id, $data );

    return [ 'success' => true, 'id' => $insert_id, 'email_sent' => (bool) $email_sent ];
}
