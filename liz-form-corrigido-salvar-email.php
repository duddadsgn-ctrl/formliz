...
// ═══════════════════════════════════════════════════════
// REST API + AJAX — salva inscrição e envia e-mail
// ═══════════════════════════════════════════════════════
...
add_action( 'wp_ajax_lizform_submit',        'lizform_ajax_submit' );
add_action( 'wp_ajax_nopriv_lizform_submit', 'lizform_ajax_submit' );

function lizform_api_submit( WP_REST_Request $req ) {
    $d = $req->get_json_params();
    if ( ! is_array( $d ) ) $d = [];

    $result = lizform_process_submission( $d );
    if ( is_wp_error( $result ) ) return $result;

    return $result;
}

function lizform_ajax_submit() {
...
function lizform_process_submission( array $d ) {
    ...
    if ( empty( $data['nome'] ) && empty( $data['email'] ) ) {
        return new WP_Error( 'invalid', 'Dados insuficientes para salvar a inscrição.', [ 'status' => 400 ] );
    }

    $table = $wpdb->prefix . 'liz_submissions';

    // Garante que a tabela exista mesmo se o plugin foi atualizado sem reativar.
    ...
    if ( false === $ok ) {
        $msg = $wpdb->last_error ? 'Erro ao salvar no banco: ' . $wpdb->last_error : 'Erro ao salvar no banco de dados.';
        return new WP_Error( 'db', $msg, [ 'status' => 500 ] );
    }

    $insert_id = (int) $wpdb->insert_id;
    $mail_sent = lizform_send_notification_email( $data, $insert_id );
...
function lizform_send_notification_email( array $data, $insert_id ) {
    $s  = lizform_get();
    $to = 'ivanenriqueales@gmail.com';

    $nome_raw = $data['nome'] ?: 'Sem nome';
    $subject  = 'Nova inscricao Liz Form - ' . $nome_raw;

    $body  = "Nova inscrição recebida pelo formulário.\n\n";
    $body .= "========================================\n";
    $body .= "DADOS DE IDENTIFICAÇÃO\n";
    $body .= "========================================\n";

    if ( ! empty( $s['f_nome_on'] ) )   $body .= "Nome:       " . ( $data['nome']      ?: '-' ) . "\n";
    if ( ! empty( $s['f_insta_on'] ) )  $body .= "Instagram:  " . ( $data['instagram'] ?: '-' ) . "\n";
    if ( ! empty( $s['f_email_on'] ) )  $body .= "E-mail:     " . ( $data['email']     ?: '-' ) . "\n";
    if ( ! empty( $s['f_cidade_on'] ) ) $body .= "Cidade/UF:  " . ( $data['cidade']    ?: '-' ) . "\n";
    if ( ! empty( $s['f_idade_on'] ) )  $body .= "Idade:      " . ( $data['idade']     ?: '-' ) . "\n";
    if ( ! empty( $s['f_prof_on'] ) )   $body .= "Profissão:  " . ( $data['profissao'] ?: '-' ) . "\n";
    ...
    for ( $i = 1; $i <= 5; $i++ ) {
        if ( empty( $s[ "q{$i}_on" ] ) ) continue;
        $label = lizform_clean_label( $s[ "q{$i}_text" ] ?? "Pergunta {$i}" );
        $resp  = $data[ 'pergunta_' . $i ] ?: '-';
        $body .= "\n[Pergunta {$i}] {$label}\n";
        $body .= "Resposta: {$resp}\n";
    }
...
(function(){
  ...
  var API='<?= $apiurl ?>', AJAX='<?= $ajaxurl ?>', RURL='<?= $rurl ?>', RDEL=<?= $rdel ?>;
  var TC='<?= $cls ?>';
  ...
  function postJSON(url,data){
    return fetch(url,{
      ...
      return resp.text().then(function(text){
        var json={};
        try{json=text?JSON.parse(text):{};}catch(e){throw new Error('Resposta inválida do servidor.');}
        if(!resp.ok||json.success===false){
          var msg=(json.data&&json.data.message)||json.message||'Erro ao salvar a inscrição.';
          throw new Error(msg);
        }
        return json;
      });
  ...
  function submit(){
    ...
    document.querySelectorAll('.lf-ok').forEach(function(b){b.disabled=true;b.style.opacity='.7';});
    ...
    // Primeiro tenta pela REST API. Se o servidor bloquear /wp-json/, tenta admin-ajax.php.
    postJSON(API,data).catch(function(restErr){
      console.warn('[LizForm REST]',restErr);
      return postJSON(AJAX+'?action=lizform_submit',data);
    }).then(function(){
      goTo(SUCCESS);
      ...
      submitting=false;
      document.querySelectorAll('.lf-ok').forEach(function(b){b.disabled=false;b.style.opacity='';});
      console.error('[LizForm]',err);
      alert('Não foi possível salvar sua inscrição agora. Por favor, tente novamente. Detalhe: '+err.message);
    });
  }
