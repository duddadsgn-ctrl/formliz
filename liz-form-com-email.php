...
// Mantém o formulário original e apenas envia um e-mail
// com os dados e respostas após salvar no banco.
// ═══════════════════════════════════════════════════════
function lizform_send_email_notification( $d, $insert_id = 0 ) {
    if ( ! is_array( $d ) ) {
        $d = [];
    }
    ...
    // Altere aqui o e-mail que deve receber as inscrições.
    ...
    $subject_name = $nome ? $nome : 'Sem nome';
    $subject      = 'Nova inscrição — ' . html_entity_decode( wp_strip_all_tags( $s['welcome_tag'] ), ENT_QUOTES, 'UTF-8' ) . ' | ' . $subject_name;

    $body  = "Olá!\n\n";
    $body .= "Uma nova inscrição acabou de ser preenchida no formulário Liz Form.\n\n";
    $body .= "────────────────────────────────────────\n";
    $body .= "DADOS DE IDENTIFICAÇÃO\n";
    $body .= "────────────────────────────────────────\n";

    if ( ! empty( $s['f_nome_on'] ) ) {
        $body .= "Nome: " . ( $nome ?: '-' ) . "\n";
    }
    if ( ! empty( $s['f_insta_on'] ) ) {
        $body .= "Instagram: " . ( $instagram ?: '-' ) . "\n";
    }
    if ( ! empty( $s['f_email_on'] ) ) {
        $body .= "E-mail: " . ( $email ?: '-' ) . "\n";
    }
    if ( ! empty( $s['f_cidade_on'] ) ) {
        $body .= "Cidade/Estado: " . ( $cidade ?: '-' ) . "\n";
    }
    if ( ! empty( $s['f_idade_on'] ) ) {
        $body .= "Idade: " . ( $idade ?: '-' ) . "\n";
    }
    if ( ! empty( $s['f_prof_on'] ) ) {
        $body .= "Profissão/Área: " . ( $profissao ?: '-' ) . "\n";
    }
    ...
    for ( $i = 1; $i <= 5; $i++ ) {
        ...
        $pergunta = html_entity_decode(
            wp_strip_all_tags( $s[ "q{$i}_text" ] ),
            ENT_QUOTES,
            'UTF-8'
        );

        $resposta = sanitize_textarea_field( $d[ 'pergunta_' . $i ] ?? '' );
    ...
    $body .= "ID interno: #" . intval( $insert_id ) . "\n";
    $body .= "Painel: " . admin_url( 'admin.php?page=liz-form' ) . "\n";
    $body .= "Site: " . home_url() . "\n";

    // Usa o domínio do próprio site como remetente para reduzir chance de bloqueio.
    ...
    if ( ! $sent ) {
        error_log( '[Liz Form] Falha ao enviar e-mail da inscrição #' . intval( $insert_id ) );
    }
...
function lizform_api_submit( WP_REST_Request $req ) {
    ...
    lizform_send_email_notification( $d, $wpdb->insert_id );
